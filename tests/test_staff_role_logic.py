...
def test_save_staff_roles(value_list, ad):
    ...
    assert message == "0 Staff Role changes saved"

    # 2. Add a new role
    new_role = {'Service Code': 'SC2', 'Role Code': 'R2', 'Bank': 0, 'Nightshift': 1}
    ...
    assert message == "1 Staff Role changes saved"
    db_sr = ad.md.find_two('Staff Role', 'SC2', 'Service Code', staff_name, 'Staff Name')
    assert db_sr > -1
    assert ad.md.get('Staff Role', 'Role Code', db_sr) == 'R2'

    # 3. Update an existing role
    updated_value_list[0]['Role Code'] = 'R2'
    role_list, bank_list, nightshift_list = create_role_lists_from_value_list(ad, updated_value_list)
    input_valid, num_changes, message = StaffRoleLogic(ad).save_staff_roles(staff_name, role_list,
                                                                            bank_list, nightshift_list)
    assert input_valid is True
    assert num_changes == 1
    assert message == "1 Staff Role changes saved"
    db_sr = ad.md.find_two('Staff Role', 'SC1', 'Service Code', staff_name, 'Staff Name')
    assert ad.md.get('Staff Role', 'Role Code', db_sr) == 'R2'

    # 4. Delete a role
    deleted_value_list = [r for r in updated_value_list if r['Service Code'] != 'SC2']
...
def test_filter_staff_names(ad):
    ...
    filtered_names = StaffRoleLogic(ad).filter_staff_names("Ja")
    assert "Jane Smith" in filtered_names
    assert "John Doe" not in filtered_names
