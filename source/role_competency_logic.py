...
class RoleCompetencyLogic:
    ...
    def save_role_competencies(self, service_code: str, db_c_list: list, db_r_list: list, value_rc: list) -> int:
        ...
        Args:
            ...
            value_rc: A 2D list of flag values.
        ...
        for c, db_c in enumerate(db_c_list):
            ...
            for r, db_r in enumerate(db_r_list):
                ...
                # If there is no record for Role Competency combination and its checkbox is checked then add it
                else:
                    if value_rc[c][r]:
                        number_changes += 1
                        self.ad.master_updated = True
